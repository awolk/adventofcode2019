...
fn test_amps(amp_control_software: &str, phase_setting: [i32; 5]) -> Result<i32, &'static str> {
...
fn permutations() -> impl Iterator<Item = [i32; 5]> {
...
fn part1(amp_control_software: &str) {
    let best_output = permutations()
        .map(|phase_setting| test_amps(amp_control_software, phase_setting).expect("failed test"))
        .max()
